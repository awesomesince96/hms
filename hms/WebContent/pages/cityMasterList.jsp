...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-10">
                    <h1 class="">City Master List</h1>
                </div>
                <div class="col-lg-2 panel-body">
                	<a href="./cityMasterAdd.jsp">
								<span class="pull-left btn btn-primary" style="margin-top: 10px;"><i class="fa fa-plus" style="margin-right: 10px;"></i>Add City</span>
                    ...
                    <div class="panel panel-default">
                        <div class="panel-body">
                            <div class="table-responsive" style="overflow-x: hidden;">
                                <table class="table center table-hover table-bordered " id="cityListTable">
                                    ...
                                    <tbody id="cityListTableBody" class="scrollContent">
                                    </tbody>
                                </table>
                            </div>
                            <!-- /.table-responsive -->
...
//    			alert(JSON.stringify(response));
    ...
    function setDataOnTable(response){
		 $("#cityListTable").dataTable().fnDestroy();
	        var tableBodyhtml=""; 
	        $.each(response, function() {
	            tableBodyhtml += getHtml(this);
	        });
	        $("#cityListTableBody").html(tableBodyhtml);
...
//	             "bScrollCollapse": false
	    ...
	    function getHtml(city) {
	        ...
	        var actionButtons = "";
	        if(status === "Inactive") {
	        	var statusButton = '<button type="button" title="Active" onClick="activeAction(\''+ city.id+ '\');"'
	            + 'class="btn btn-info btn-sm shiny icon-only azure"><i class="fa fa-check"></i></button>';
	        } 
	        else {
	        	var statusButton = '<button type="button" title="Inactive" onClick="inactiveAction(\''+ city.id+ '\');"'
	            + 'class="btn btn-info btn-sm shiny icon-only azure"><i class="fa fa-times"></i></button>';
	        }
	        
	        var editButton = ' <button type="button" title="Manage Appointment" onClick="cityEditAction(\''+ city.id+ '\');"'
	    ...
	    function cityEditAction(id){
	    	window.location.href = "./cityMasterAdd.jsp?id="+id;
	    }
